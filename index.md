...
Alternatively, set it manually with
[`eia_set_key()`](https://docs.ropensci.org/eia/reference/eia_key.md) in
the current R session. Further, it can always be passed explicitly to
the `key` argument of a given `eia` function.
...
![](reference/figures/README-plt-1.png)
